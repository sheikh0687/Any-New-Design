...
class PopUpApprovalVC: UIViewController {
    ...
    func GetProfile() {
        ...
        print(paramsDict)
        CommunicationManeger.callPostService(apiUrl: Router.get_OutletAdmin_AuthrisedApprover.url(), parameters: paramsDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func webDeletShift() {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.change_set_shift_status.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async { [self] in
                let swiftyJsonVar = JSON(responseData)
                print(swiftyJsonVar)
                if(swiftyJsonVar["status"].stringValue == "1") {
