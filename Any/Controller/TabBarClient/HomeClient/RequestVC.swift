...
extension RequestVC {
    ...
    func WebGetApprovedBooking() {
        ...
        print(paramsDict)
        CommunicationManeger.callPostService(apiUrl: Router.get_set_shift_book_by_date.url(), parameters: paramsDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func WebRejectBooking(strCard:String) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.change_set_shift_status.url(), parameters: paramsDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func webDeletShift(strSt:String) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.delete_my_shifts.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async {
                let swiftyJsonVar = JSON(responseData)
                print(swiftyJsonVar)
                if(swiftyJsonVar["status"].stringValue == "1") {
