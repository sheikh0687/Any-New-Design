...
class UserChat: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextViewDelegate {
    ...
    func wsSendMessage()  {
        ...
        print(paramDict)
        CommunicationManeger.callPostService(apiUrl: Router.insert_chat.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func wsGetChatAgain()  {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.get_chat_detail.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async {
                let swiftyJsonVar = JSON(responseData)
                print(swiftyJsonVar)
                if(swiftyJsonVar["status"].stringValue == "1") {
