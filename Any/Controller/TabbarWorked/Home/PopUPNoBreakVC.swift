...
class PopUPNoBreakVC: UIViewController {
    ...
    func webbrakType() {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.add_break_time.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async { [self] in
                let swiftyJsonVar = JSON(responseData)
                print(swiftyJsonVar)
