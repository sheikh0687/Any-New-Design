...
class BookingDetailVC: UIViewController,FooTwoViewControllerDelegate {
    ...
    func WebAddClockIn(strType:String) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.add_clock_in_time.url(), parameters: paramsDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func WebGetBookingDetail() {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.get_set_shift_cart_details.url(), parameters: paramsDict, parentViewController: self, successBlock: { (responseData, message) in
