...
class SameDayRequestVC: UIViewController, FooTwoViewControllerDelegate {
    ...
    func WebRejectBooking(strCard:String) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.add_to_set_shift_cart_broadcast.url(), parameters: paramsDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func WebGetApprovedBooking() {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.get_broadcast_shift.url(), parameters: paramsDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func webDeletShift(strSt:String) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.shift_rejected_by_worker.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async { [self] in
                let swiftyJsonVar = JSON(responseData)
                print(swiftyJsonVar)
                if(swiftyJsonVar["status"].stringValue == "1") {
