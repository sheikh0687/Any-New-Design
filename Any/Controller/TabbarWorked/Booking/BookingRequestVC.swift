...
extension BookingRequestVC {
    ...
    func webAddToCartSubmit(_ shiftId: String,_ shiftRate: String) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.add_to_set_shift_cart.url(), parameters: paramsDict,  parentViewController: self, successBlock: { (responseData, message) in
    ...
    func getDataGetList() {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.get_shift_by_day_week_count.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func getDataShiftAvailble() {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.get_shift_by_day.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async {
                let swiftyJsonVar = JSON(responseData)
                if(swiftyJsonVar["status"].stringValue == "1") {
    ...
    func addBookingFinal(_ cartID: Int) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.add_set_shift_book.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async {
                let swiftyJsonVar = JSON(responseData)
                if(swiftyJsonVar["status"].stringValue == "1") {
    ...
    func webDeletShift(strCartiD:String) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.change_set_shift_status_worker_side.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async {
                let swiftyJsonVar = JSON(responseData)
                if(swiftyJsonVar["status"].stringValue == "1") {
