...
class NewBookingVC: UIViewController,UITextFieldDelegate  {
    ...
    func getListOfDatesShift() {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.get_days_List.url(), parameters: paramsDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func getClientList(strJobiD: String, strJobName: String) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.get_client_list.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async {
                let swiftyJsonVar = JSON(responseData)
                if(swiftyJsonVar["status"].stringValue == "1") {
    ...
    func getLikeUnlike(strClientiD:String) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.like_unlike.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async {
                let swiftyJsonVar = JSON(responseData)
                if(swiftyJsonVar["status"].stringValue == "1") {
