...
class listAdminVC: UIViewController {
    ...
    func getDataGetChatList() {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.get_OutletAdmin_AuthrisedApprover.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func webDeletShift(strSt:String) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.delete_OutletAdmin_AuthrisedApprover.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async {
                let swiftyJsonVar = JSON(responseData)
                print(swiftyJsonVar)
                if(swiftyJsonVar["status"].stringValue == "1") {
