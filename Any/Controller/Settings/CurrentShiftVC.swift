...
class CurrentShiftVC: UIViewController {
    ...
    func getDataGetChatListd(strType:String) {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.update_booking_status_profile.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func getDataGetList() {
        ...
        CommunicationManeger.callPostService(apiUrl: Router.get_my_set_shift.url(), parameters: paramDict, parentViewController: self, successBlock: { (responseData, message) in
            DispatchQueue.main.async {
                let swiftyJsonVar = JSON(responseData)
                print(swiftyJsonVar)
                if(swiftyJsonVar["status"].stringValue == "1") {
