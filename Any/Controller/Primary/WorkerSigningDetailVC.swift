...
extension WorkerSigningDetailVC {
    ...
    //        showProgressBar()
    //
    //        CommunicationManeger.uploadImagesAndData(apiUrl: Router.signUp.url(), params: (paramSignupDict as! [String : String]) , imageParam: paramImages, videoParam: [:], parentViewController: self, successBlock: { (responseData, message) in
    //
    //            DispatchQueue.main.async { [self] in
    //                let swiftyJsonVar = JSON(responseData)
    //                if(swiftyJsonVar["status"].stringValue == "1") {
