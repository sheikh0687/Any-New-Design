...
extension LoginVC {
    ...
    func WebGetCountryList() {
        ...
        print(paramsDict)
        CommunicationManeger.callPostService(apiUrl: Router.get_country_list.url(), parameters: paramsDict, parentViewController: self, successBlock: { (responseData, message) in
    ...
    func WebVerifyNumber() {
        ...
        CommunicationManeger.uploadImagesAndData(apiUrl: Router.verify_number.url(), params: (paramsDict as! [String : String]) , imageParam: [:], videoParam: [:], parentViewController: self, successBlock: { (responseData, message) in
