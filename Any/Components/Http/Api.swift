...
class Api: NSObject {
    ...
    func add_Card(_ vc: UIViewController, _ params: [String: AnyObject], _ success: @escaping(_ responseData : AnyObject) -> Void) {
        CommunicationManeger.callPostService(apiUrl: Router.save_CardStripe.url(), parameters: params, parentViewController: vc, successBlock: { (response, message) in
            success(response)
        }) { (error) in
    ...
    func delete_SavedCard(_ vc: UIViewController, _ params: [String: AnyObject], _ success: @escaping(_ responseData : AnyObject) -> Void) {
        CommunicationManeger.callPostService(apiUrl: Router.delete_SaveCard.url(), parameters: params, parentViewController: vc, successBlock: { (response, message) in
            success(response)
        }) { (error) in
            vc.alert(alertmessage: error.localizedDescription)
        }
